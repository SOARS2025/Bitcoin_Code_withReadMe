...
def plot_volume_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Time", yaxis_title="Volume", title="Trading Volume")
    return fig

def plot_obv_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data.index, y=data['OBV'], mode="lines", name="OBV"))
    fig.update_layout(xaxis_title="Time", yaxis_title="OBV", title="On-Balance Volume (OBV)")
    return fig
...
def plot_candlestick_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Time", yaxis_title="Price (USD)", title="Candlestick Chart with Bollinger Bands")
    return fig
...
def plot_ma_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Time", yaxis_title="Price (USD)", title="Price with SMA & EMA")
    return fig
...
def plot_rsi_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Time", yaxis_title="RSI", title="Relative Strength Index (RSI)",
...
def plot_macd_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Time", yaxis_title="MACD", title="MACD Indicator")
    return fig
...
def run_market_analysis_tab():
    ...
    stock_data = {}
    ticker = 'AAPL'
    data = yf.download(ticker, period='1d', multi_level_index=False)
    if not data.empty:
...
def plot_volume_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Bar(x=data['Time'], y=data['Volume'], name="Volume"))
    fig.update_layout(xaxis_title="Time", yaxis_title="Volume", title="Trading Volume")
    return fig

def plot_obv_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data['Time'], y=data['OBV'], mode="lines", name="OBV"))
    fig.update_layout(xaxis_title="Time", yaxis_title="OBV", title="On-Balance Volume (OBV)")
    return fig
...
def plot_candlestick_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure(data=[go.Candlestick(
        ...
        low=data['Low'], close=data['Close'], name="Candlestick")])
    fig.add_trace(go.Scatter(x=data['Time'], y=data['BB_Upper'], line=dict(color='rgba(255,0,0,0.5)'), name="BB Upper"))
    fig.add_trace(go.Scatter(x=data['Time'], y=data['BB_Middle'], line=dict(color='rgba(0,0,255,0.5)'), name="BB Middle"))
    fig.add_trace(go.Scatter(x=data['Time'], y=data['BB_Lower'], line=dict(color='rgba(0,255,0,0.5)'), name="BB Lower"))
    fig.update_layout(xaxis_title="Time", yaxis_title="Price (USD)", title="Candlestick Chart with Bollinger Bands")
    return fig
...
def plot_ma_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=data['Time'], y=data['SMA_14'], mode='lines', name="SMA 14"))
    fig.add_trace(go.Scatter(x=data['Time'], y=data['EMA_14'], mode='lines', name="EMA 14"))
    fig.update_layout(xaxis_title="Time", yaxis_title="Price (USD)", title="Price with SMA & EMA")
    return fig

def plot_rsi_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data['Time'], y=data['RSI_14'], mode='lines', name='RSI'))
    fig.add_hline(y=70, line_dash='dash', line_color='red', annotation_text="Overbought")
    fig.add_hline(y=30, line_dash='dash', line_color='green', annotation_text="Oversold")
    fig.update_layout(xaxis_title="Time", yaxis_title="RSI", title="Relative Strength Index (RSI)",
                      yaxis=dict(range=[data['RSI_14'].min()-5, data['RSI_14'].max()+5]))
...
def plot_macd_chart(data: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=data['Time'], y=data['MACD'], mode='lines', name='MACD Line'))
    fig.add_trace(go.Scatter(x=data['Time'], y=data['MACD_Signal'], mode='lines', name='Signal Line'))
    fig.add_trace(go.Bar(x=data['Time'], y=data['MACD_Hist'], name='MACD Histogram', opacity=0.5))
    fig.update_layout(xaxis_title="Time", yaxis_title="MACD", title="MACD Indicator")
    return fig
